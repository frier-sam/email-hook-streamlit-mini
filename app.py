    ...
    if not st.session_state.logged_in:
        ...
        if st.button("Login", key="login_button"):
            if username in USER_CREDENTIALS and USER_CREDENTIALS[username] == password:
                ...
                if remember_me:
                    cookie_manager.set("logged_in", "true", key="set_logged_in")
        ...
        if st.button("Logout", key="login_page_logout"):
            cookie_manager.delete("logged_in", key="delete_logged_in_login")
            cookie_manager.delete("username", key="delete_username_login")
            st.session_state.logged_in = False
    ...
    if st.session_state.logged_in:
        ...
        with st.sidebar:
            if st.button("Logout", key="sidebar_logout"):
                cookie_manager = get_cookie_manager()
                cookie_manager.delete("logged_in", key="delete_logged_in_sidebar")
                cookie_manager.delete("username", key="delete_username_sidebar")
                st.session_state.logged_in = False
                st.session_state.username = None
                st.success("Logged out successfully!")
                st.rerun()
...
"""
    ...
    with st.expander("Advanced Settings"):
        ...
        with tab1:
            ...
            if st.button("Save Hook Template", key="save_hook_template"):
            ...
            if st.button("Reset Hook Template", key="reset_hook_template"):
        ...
        with tab2:
            ...
            if st.button("Save Fit Template", key="save_fit_template"):
            ...
            if st.button("Reset Fit Template", key="reset_fit_template"):
    ...
    if st.button("Generate Hooks", key="generate_hooks"):
    ...
    if st.session_state.generated_hooks:
        ...
        selected_url = st.selectbox("Select a website to analyze", urls, key="url_select")
        
        if st.button("Analyze Service Fit", key="analyze_fit"):
            # Initialize Gemini client
            client = genai.Client(api_key=os.environ.get("GEMINI_API_KEY"))
